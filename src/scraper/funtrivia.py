...
class FunTriviaScraper(BaseScraper):
    ...
    async def _scrape_quiz(self, quiz_url: str, stats: Dict = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Skip Photo Quiz and Audio Quiz types - focusing only on Multiple Choice and True/False
            if quiz_type in ['Photo Quiz', 'Audio Quiz']:
                self.logger.info(f"[{quiz_log_id}] Skipping {quiz_type} type - focusing on Multiple Choice and True/False only")
                return []
                
            if quiz_type not in ['Multiple Choice', 'Audio Quiz']:
    ...
    async def _play_through_complete_quiz(self, page: Page, quiz_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Step 2: Submit answers - only process Multiple Choice and True/False
            # All other quiz types should be filtered out earlier
    ...
    async def _extract_complete_results(self, page: Page, original_questions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"📊 Input: {len(original_questions)} original questions to enhance")
            ...
            if enhanced_questions and len(enhanced_questions) == len(original_questions):
                ...
                if descriptions_found > 0 or correct_answers_found > len(enhanced_questions) * 0.5:
                    ...
                    self.logger.info(f"📤 Returning {len(enhanced_questions)} questions from Strategy 1 (EARLY EXIT)")
                    ...
                    self.logger.warning(f"❌ Strategy 1 insufficient results - descriptions={descriptions_found}, answers={correct_answers_found}")
            else:
                result_len = len(enhanced_questions) if enhanced_questions else 0
                self.logger.warning(f"❌ Strategy 1 failed - returned {result_len}/{len(original_questions)} questions")
            ...
            if enhanced_questions and len(enhanced_questions) == len(original_questions):
                ...
                if descriptions_found > 0 or correct_answers_found > 0:
                    ...
                    self.logger.info(f"📤 Returning {len(enhanced_questions)} questions from Strategy 2 (EARLY EXIT)")
                    return enhanced_questions
                else:
                    self.logger.warning(f"❌ Strategy 2 insufficient results - descriptions={descriptions_found}, answers={correct_answers_found}")
            else:
                result_len = len(enhanced_questions) if enhanced_questions else 0
                self.logger.warning(f"❌ Strategy 2 failed - returned {result_len}/{len(original_questions)} questions")
            ...
            if enhanced_questions:
                ...
                if descriptions_found > 0:
                    ...
                    self.logger.info(f"📤 Returning {len(enhanced_questions)} questions from Strategy 3 (EARLY EXIT)")
                    return enhanced_questions
                else:
                    self.logger.warning(f"❌ Strategy 3 insufficient results - descriptions={descriptions_found}")
            else:
                self.logger.warning("❌ Strategy 3 failed - no questions returned")
            ...
            basic_questions = await self._enhance_questions_basic(original_questions)
            self.logger.info(f"📤 Returning {len(basic_questions)} questions from Strategy 4 (fallback)")
            return basic_questions
            
        except Exception as e:
            self.logger.error(f"💥 Error in comprehensive results extraction: {e}")
            fallback_questions = await self._enhance_questions_basic(original_questions)
            self.logger.info(f"📤 Returning {len(fallback_questions)} questions from fallback due to error")
    ...
    def _extract_funtrivia_explanation(self, text: str) -> Optional[str]:
        ...
        try:
            ...
            # Enhanced patterns to match FunTrivia's various structures
            patterns = [
                ...
                # Enhanced flexible patterns for edge cases
                r'The correct answer was\s+[^\.]+\.(.+?)(?=\d+%|I see an error|Your Answer|Question \d+|\Z)',
                r'correct answer was\s+[^\.]+\.(.+?)(?=\d+%|I see an error|Your Answer|Question \d+|\Z)',
                # Pattern without period requirement (some quizzes might not have periods)
                r'The correct answer was\s+[^.\n]+\s*(.+?)(?=\d+%\s+of\s+players|I see an error|Your Answer:|Question \d+|\Z)',
                # Capture everything after answer until common terminators
                r'The correct answer was[^.]+(?:\.|)\s*(.+?)(?=\d+%|Your Answer|Question \d+|Submit|I see an error|\Z)',
                # Very flexible fallback pattern
                r'correct answer[^.]*(?:\.|)\s*(.+?)(?=\d+%|Submit|Question \d+|\Z)',
            ]
            ...
            # Fallback 1: line-by-line analysis for edge cases
            fallback_explanation = self._extract_explanation_line_by_line(text)
            if fallback_explanation:
                return fallback_explanation
            
            # Fallback 2: Look for any substantial educational content
            return self._extract_any_educational_content(text)
    ...
    def _extract_explanation_line_by_line(self, text: str) -> Optional[str]:
        """
        Enhanced fallback method to extract explanations by analyzing line by line.
        """
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if re.search(r'The correct answer was|correct answer:|answer:|correct:', line, re.IGNORECASE):
                    found_answer = True
                    # Check if explanation is on the same line after the answer
                    answer_match = re.search(r'(?:The\s+)?correct answer (?:was\s+|:|is\s+)[^.]*\.?\s*(.+)', line, re.IGNORECASE)
                    if answer_match and len(answer_match.group(1).strip()) > 20:
                        explanation_lines.append(answer_match.group(1).strip())
                    continue
                
                # If we found the answer, start collecting explanation lines
                if found_answer:
                    # Stop at statistics, navigation, or next question
                    if re.search(r'\d+%\s+of\s+players|I see an error|Your Answer:|Question \d+|Submit|Next Question', line, re.IGNORECASE):
                        break
                    
                    # Collect substantial lines (likely part of explanation)
                    # Lowered threshold and added more content checks
                    if (len(line) > 15 and 
                        not self._is_navigation_line(line) and
                        not re.match(r'^\d+\.\s*$', line) and  # Skip question numbers
                        not re.match(r'^[A-D]\)|^[a-d]\)', line)):  # Skip option letters
                        explanation_lines.append(line)
                    
                    # If we've collected enough lines or hit a natural break, stop
                    if len(explanation_lines) > 5 or (explanation_lines and 
                                                     re.search(r'[.!?]\s*$', line)):
                        # Check if we have enough content
                        combined = ' '.join(explanation_lines).strip()
                        if len(combined) > 50:
                            break
            
            if explanation_lines:
                explanation = ' '.join(explanation_lines).strip()
                # Be more lenient on validation for this fallback method
                if len(explanation) >= 20 and len(explanation.split()) >= 5:
    ...
    def _extract_any_educational_content(self, text: str) -> Optional[str]:
        """
        Last resort: Extract any substantial educational content from the text.
        This is used when standard patterns fail but there might still be useful explanations.
        """
        try:
            # Split into sentences and look for educational content
            ...
            for sentence in sentences:
                ...
                # Skip navigation, questions, answers
                if (self._is_navigation_line(sentence) or
                    re.search(r'^\d+\.\s+', sentence) or  # Question numbers
                    re.search(r'^[A-D]\)|Your Answer:|Correct Answer:', sentence, re.IGNORECASE) or
                    re.search(r'\d+%\s+of\s+players', sentence, re.IGNORECASE)):
                    continue
                
                # Look for sentences that sound educational
                educational_indicators = [
                    r'\b(?:because|since|due to|as a result|therefore|thus|hence)\b',
                    r'\b(?:was|were|is|are|became|become)\b.*\b(?:known|famous|used|created|built|designed)\b',
                    r'\b(?:the|this|that|these|those)\b.*\b(?:originated|developed|evolved|established)\b',
                    r'\b(?:during|in|at|from|since)\b.*\b(?:century|period|era|time|year)\b',
                    r'\b(?:located|situated|found|discovered|built)\b.*\b(?:in|at|near|on)\b',
                    r'\b(?:also|additionally|furthermore|moreover|however|nevertheless)\b',
                ]
                
                # If sentence has educational content markers, include it
                if any(re.search(pattern, sentence, re.IGNORECASE) for pattern in educational_indicators):
                    educational_sentences.append(sentence)
                # Also include long descriptive sentences
                elif len(sentence) > 50 and len(sentence.split()) > 10:
                    # Check if it's not just a question or navigation
                    if not (sentence.endswith('?') or 
                           'click' in sentence.lower() or
                           'visit' in sentence.lower() or
                           'browse' in sentence.lower()):
                        educational_sentences.append(sentence)
            
            if educational_sentences:
                explanation = '. '.join(educational_sentences[:3]).strip()  # Take first 3 sentences
                if len(explanation) >= 30:
                    return explanation + ('.' if not explanation.endswith('.') else '')
            
            return None
            
        except Exception as e:
            self.logger.debug(f"Error in educational content extraction: {e}")
            return None
    ...
    async def _extract_photo_quiz_questions(self, page: Page) -> List[Dict[str, Any]]:
        ...
        try:
            questions = await page.evaluate("""
                () => {
                    const questions = [];
                    const seenQuestions = new Set(); // Track seen question numbers to prevent duplicates
                    ...
                    questionElements.forEach((qEl, index) => {
                        ...
                        if (questionMatch) {
                            ...
                            if (questionText && options.length >= 2) {
                                seenQuestions.add(questionNumber); // Mark as seen
                                questions.push({
