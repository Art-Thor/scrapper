...
class CSVHandler:
    ...
    def get_existing_question_texts(self, csv_file: str) -> Set[str]:
        """Get existing question texts from a CSV file to prevent content duplicates."""
        csv_path = self.output_dir / csv_file
        existing_texts = set()
        ...
        if csv_path.exists():
            try:
                ...
                if 'Question' in df.columns:
                    # Normalize question texts for comparison
                    existing_texts = set(df['Question'].dropna().astype(str).str.strip().str.lower())
                    self.logger.debug(f"Found {len(existing_texts)} existing question texts in {csv_file}")
            except Exception as e:
                self.logger.error(f"Error reading existing question texts from CSV {csv_file}: {e}")
        
        return existing_texts

    def filter_new_questions(self, questions: List[Dict[str, Any]], csv_file: str) -> List[Dict[str, Any]]:
        """Filter out questions that already exist in the CSV file (by Key OR by question text)."""
        existing_keys = self.get_existing_keys(csv_file)
        existing_texts = self.get_existing_question_texts(csv_file)
        
        if not existing_keys and not existing_texts:
            return questions
        ...
        for question in questions:
            ...
            if question_key in existing_keys:
                self.logger.debug(f"Skipping duplicate question by Key: {question_key}")
                continue
            
            # Check for duplicate by question text content
            if question_text in existing_texts:
                self.logger.debug(f"Skipping duplicate question by content: {question_text[:50]}...")
                continue
            
            # Question is new - add it and track its text to prevent duplicates within this batch
            new_questions.append(question)
            existing_texts.add(question_text)
        
        self.logger.info(f"Filtered {len(questions)} questions to {len(new_questions)} new questions (removed key + content duplicates)")
